...
| [`code/starvla_lab/`](code/starvla_lab/) | [10 · 改进方案](reports/10_improvement_plan.md) 阶段 A 的研究包：`probes/`（跨头探针、CKA、漂移追踪）、`schedules/`（分层学习率衰减、漂移驱动的冻结与辅助数据调度）、`heads/`（未来特征预测头、关键帧头、`QwenMultiHeadLab`）、`data/`（按轨迹子采样、辅助头离线数据）、`train/`（`train_starvla_lab.py` 入口）、`bench/`（"只换骨干"协议、开销测量、头 dropout）、`configs/`（R0–R9 矩阵） | 128 个 CPU 测试 |
...
| F2 · 冻结骨干迁移（16 条运行，1×A100） | 四种骨干（预训练 / OFT 微调 / 三头微调 / OFT+冻结嵌入）全部冻结，各接全新 OFT 头与 PI 头在未见的 LIBERO-spatial 与已调的 LIBERO-goal 上训 300 步：没有有害的 decoder lock-in（OFT 骨干接新 PI 头 = 预训练骨干，0.338 vs 0.334）；三头骨干对同任务新 OFT 头最好（0.216 vs 0.226 vs 0.267）但优势不迁到新场景；PI 头对骨干不敏感。**fp32 骨干复测**：新 OFT 头在未见的 spatial 上 0.277 → 0.214（OFT fp32）→ 0.211（三头 fp32），goal 0.267 → 0.173 → 0.169——训练到位的骨干可复用且迁到新场景，三头略优 | [`experiments/results/f2_frozen_backbone_transfer/`](experiments/results/f2_frozen_backbone_transfer/README.md) |
| F3 · 三头模型的学习率控制（2 条运行，1×A100） | 漂移驱动 LLRD 端到端工作：第 35 层越过阈值后倍率减半到下限，最终漂移 0.039 vs 0.052，头损失代价 0.004–0.011，但触发太晚（阈值应 1e-3）；静态 LLRD 0.85 不硬冻结明显更差。附带发现单卡 bf16 路径无 fp32 主权重、小更新被舍入——加了 `trainer.lab.backbone_fp32`，见 F4 | [`experiments/results/f3_llrd/`](experiments/results/f3_llrd/README.md) |
| F4 · fp32 主权重校准（2 条运行，1×A100） | 同配置下把可训练骨干参数升 fp32（计算仍 bf16）：单头 OFT 损失 0.247 → 0.183，三头 0.242/0.324/0.416 → 0.178/0.293/0.353；第 35 层漂移 OFT 0.0003 → 0.026、三头 0.052 → 0.484——单卡 bf16 路径此前一直在舍掉骨干更新，三头/单头顶层改写倍数由 240× 修正为 19×，"三头不伤 OFT 头"仍成立；fp32 checkpoint 上的跨头探针：H1 查询位可读性 0.462 → 0.562（OFT）→ 0.581（三头），跨场景迁移 −1.5 → −0.08 → −0.03，保留度 1.0 → 0.87 → 0.79——微调把动作信息写进骨干、三头写得更多更可迁移，bf16 上"无写入"的结论作废 | [`experiments/results/f4_fp32_master_weights/`](experiments/results/f4_fp32_master_weights/README.md) |
| robocore · LIBERO 真机评测（tianyiyun A100 #5，官方 `StarVLA/Qwen3-VL-OFT-LIBERO-4in1`，OSMesa 软件渲染） | `robocore eval` 端到端跑通 StarVLA policy server + LIBERO harness：spatial 100.0 / object 99.0 / goal 98.0 / libero_10 93.0，平均 97.5（10 次/任务，上游 50 次报 96.6）——桥接层与评测编排第一次被真实 checkpoint 验证 | [robocore `experiments/results/libero/`](https://github.com/asimfish/robocore/tree/main/experiments/results/libero) · [支持矩阵](https://github.com/asimfish/robocore/blob/main/docs/SUPPORT_MATRIX.md) |
...
仍需 GPU：单卡队列里的 F5（LIBERO-goal+spatial × 2000 步 fp32、探针含未见 object、迁移到 object）、F3 fp32 复跑、F2 新 PI 头 fp32 复测（F5 首跑 09-09 因 StarVLA 数据管线的 PyAV/dav1d 解码线程泄漏死锁于 374 步，已用 `trainer.lab.decoder_gc_every` + 运行停滞看门狗修复并于 09-12 重排，见 [`experiments/results/f5_libero_2suite_2k/`](experiments/results/f5_libero_2suite_2k/README.md)）、LIBERO / RoboTwin 仿真评测、DeepSpeed 多卡显存、R3 标定曲线、任何 ≥ 10k 步的训练效果数字——这些是 [07 · 路线图](reports/07_research_roadmap.md) 第 1 个月"复现 VLAct"的起点。

### 3.2 运行测试

```bash
python3 -m pytest code/vlact_ext/tests -q        # 60 passed, 1 skipped（系统 python3.9，CPU）
python3 -m pytest code/starvla_lab/tests -q      # 128 passed
...
GPU 侧：`scripts/gpu_overhead_bench.py`、`scripts/probe_diagnostics.py`、`scripts/cross_head_probe.py` 与 `scripts/cluster/{sync_to_node,setup_gpu_env,run_overhead_bench,run_f0_smoke,run_cross_head_probe,run_f2_transfer,run_f3_llrd,run_f4_fp32}.sh` 可在任一有 Qwen3-VL-4B 权重的单卡机器上复现 §3.1 的数字。
...
│   ├── starvla_lab/                # 改进方案研究包（probes / schedules / heads / data / train / bench / configs）+ 128 个测试
