...
> **已被 F4 取代（2026-09-06）**：本节的 checkpoint 都来自单卡 bf16 路径，骨干更新大部分被舍入吞掉（[F4](../f4_fp32_master_weights/README.md)）。在 fp32 主权重的 checkpoint 上，微调后的可读性明显上升（查询位 0.462 → 0.562 / 0.581）、三头骨干跨场景迁移最好、保留度降到 0.87 / 0.79。下面保留原文作为 bf16 读数的记录与方法学参考。
