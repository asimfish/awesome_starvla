...
`trainer.lab.mode` 为 `single` / `cotrain` / `auto`；`trainer.lab.{llrd, aux_scheduler, probes, head_dropout}` 的全部字段见 `train/lab_config.py`；`trainer.lab.backbone_fp32: true` 把可训练的骨干参数升到 fp32（计算仍走 bf16 autocast）——单卡无 DeepSpeed 时 StarVLA 直接在 bf16 权重上做 AdamW 更新，lr 1e-5 的更新对多数权重不足半个 ulp 会被舍掉，这个开关对应 DeepSpeed 路径的 fp32 主权重；`trainer.lab.decoder_gc_every`（默认 1，0 关闭）在 StarVLA 的 LeRobot 视频解码后调用 `gc.collect(1)`——PyAV 容器是引用环，`close()` 不会释放 libdav1d 的线程池，DataLoader worker 里每次解码泄漏 16 个 `dav1d-worker` 线程，F5 跑到 374 步时每个 worker 积累 2000–3200 个线程后死锁（`data/decoder_gc.py`，节点实测补丁后 96 次解码线程数不变、每次 2 ms）；`datasets.vla_data.data_mix` 可写内联 `目录:机器人[,…]`。所有 `trainer.lab.*` 键只被这个入口消费，StarVLA 原生脚本会忽略它们。
