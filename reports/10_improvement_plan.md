...
| **单卡数据管线的解码线程泄漏（F5 实测，2026-09-09）** | StarVLA `gr00t_lerobot/video.py` 的 `torchvision_av` / `pyav` 后端每次解码开一个 PyAV 容器；`InputContainer ↔ Stream ↔ CodecContext` 是引用环，`container.close()` 释放不了 libdav1d 的线程池（128 核节点上每个解码器 16 个 `dav1d-worker` 线程），要等分代 GC 的第 1/2 代回收；DataLoader worker 堆大、这类回收稀少，F5 OFT 跑到 374 步时 4 个 worker 各积累 2000–3200 个线程后在 `poll` 里死锁，主进程无超时地等 worker 队列，GPU 0% 占卡 69 小时。节点实测：不回收每次解码 +16 线程，解码后 `gc.collect(1)` 线程数持平、每次 2 ms（解码本身 20 ms）。对策：`trainer.lab.decoder_gc_every`（默认开，`starvla_lab/data/decoder_gc.py` 在 fork worker 之前包住 `datasets.py` 绑定的解码函数）；`scripts/cluster/run_with_stall_guard.sh` 对日志 20 分钟无变化的运行整组杀掉、F5 自动重试一次；F0–F4 各 ≤ 500 步未触发，结果不受影响 |
...
| VLAct 配方 (c) 多头共监督 | 三头不伤单头、骨干更换头友好 | 三头模型里 OFT 头损失与单头相同（fp32：0.178 vs 0.183）、开销 1.45–1.54×；fp32 下三头骨干的查询位表征跨场景迁移最好（goal→spatial −0.03 vs OFT −0.08），pooled 可读性最高（0.566 vs 0.537）。F2 fp32 复测（新 OFT 头）：预训练 0.277 → OFT fp32 0.214 → 三头 fp32 0.211（未见的 spatial），goal 0.267 → 0.173 → 0.169——训练到位的骨干可复用且迁到新场景，三头略优；新 PI 头的复测等卡 | "多头共监督让写入的表征变通用"在小规模上有了第一条正面证据（预注册指标）；R3 vs R1 用同一探针 + F2 协议放大检验 |
...
**下一步（按优先级）**：F5（goal+spatial × 2000 步、fp32，OFT vs 三头，探针含未见的 object，再按 F2 协议迁移到 object；配置 `f5_libero_2suite_2k.yaml`，与 F3 fp32 复跑、F2 新 PI 头 fp32 复测排成单卡队列 `run_queue_20260907.sh`，首跑 09-09 因解码线程泄漏死锁于 374 步，修复后 09-12 重新排队）→ R3（VLAct 全配方、LIBERO 四套 + caption 共训、DeepSpeed）用新口径标定，同时用 F2 协议做 G2 的第一批点。
